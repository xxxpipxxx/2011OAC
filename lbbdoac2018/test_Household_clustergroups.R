...
preclusterV1PER <- fread("Pre-Cluster Data/01_RM2018_HouseholdPercentages.csv")
preclusterV1COUNT <- fread("2018_Household_Raw_Variables.csv")
...
clusterV1  <- fread("Cluster Data/RM2018_Household_Converted_Transformed_Range_1000_KMeans_Runs.csv")
clusterV1summary <- fread("Cluster Data/RM2018_Household_Converted_Transformed_Range_Cluster_Metadata_1000_Runs.csv")
hist(preclusterV1PER$RM2018_01_PCT)
...
compare_clusters <- preclusterV1COUNT %>% 
  left_join(clusterV1 %>% select(UPRN = OA, Cluster))
...
if(RQOUTPUT=="YES")
{
  ...
  write.table(RM2018_OAC_Input_PCT_RATIO, paste("Clustergroups/01_RM2018_Householdclusters_Percentages.csv", sep = ""), sep = ",", row.names= FALSE, col.names = TRUE, qmethod = "double")
}

cluster_groups <- fread("Clustergroups/01_RM2018_Householdclusters_Percentages.csv")
